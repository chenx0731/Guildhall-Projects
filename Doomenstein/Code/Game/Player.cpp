...
void Player::UpdateInput(float deltaSeconds)
{
	...
	if (m_type == ControllerType::KEYBOARD) {
		...
		if (tempController.GetRightTrigger() > 0.2f) {
			/*RaycastResult3D result = gameState->m_currentMap->RaycastAll(m_position, x_forward, 10.f);
			DebugAddWorldLine(m_position, m_position + 10.f * x_forward, 0.01f, 10.f, Rgba8::WHITE, Rgba8::WHITE, DebugRenderMode::X_RAY);
			DebugAddWorldPoint(result.m_impactPos, 0.06f, 10.f);
			DebugAddWorldArrow(result.m_impactPos, (result.m_impactPos + 0.3f * result.m_impactNormal), 0.03f,
				10.f, Rgba8::BLUE, Rgba8::BLUE, DebugRenderMode::X_RAY);*/
			if (actor->m_weapons.size() > 0) {
				actor->m_weapons[actor->m_currentWeaponIndex]->Fire(actor);
			}
		}
	...
	if (m_cameraMode == CameraMode::First_Person) {
		...
		if (actor != nullptr) {
			...
			else
			{
				...
				if (actor->m_position.z > level + 1.f || actor->m_position.z < level)
				{
					/*actor->m_position.z = level;
					m_position.z = level + actor->m_actorDef->m_eyeHeight;*/
				}
				else
				{
