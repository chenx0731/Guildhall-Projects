...
{
	LoadAssets();
	//m_players[0] = new Player(nullptr);
	//m_players[1] = new Player(nullptr);
	//m_playerNum = 1;
...
void GamePlaying::Update()
{
	...
	for (int playerIndex = 0; playerIndex < 2; playerIndex++) {
		if (m_players[playerIndex]) {
			...
			Map* map = GetMapByLevel(actor->m_position.z);
			if (map != m_currentMap) {
				AddPlayerToMap(playerIndex, map);
...
void GamePlaying::Enter()
{
	...
	m_currentMap = m_maps[defaultMap];
	for (int playerIndex = 0; playerIndex < 2; playerIndex++) {
		if (m_players[playerIndex])
...
Map* GamePlaying::GetMapByLevel(float level)
{
	for (int mapIndex = 0; mapIndex < m_mapNames.size(); mapIndex++) {
		//Map* map = new Map(this, m_mapNames[mapIndex].c_str());
	...
	for (int playerIndex = 0; playerIndex < 2; playerIndex++) {
		if (m_players[playerIndex]) {
			...
			for (int mapIndex = 0; mapIndex < m_mapNames.size(); mapIndex++) {
				//Map* map = new Map(this, m_mapNames[mapIndex].c_str());
				Map* const& map = m_maps.at(m_mapNames[mapIndex]);
				map->Render(m_players[playerIndex]->m_worldCamera);
				//m_maps.push_back(map);
			}
