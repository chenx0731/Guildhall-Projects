...
SpriteSheet*			g_basicSprite = nullptr;
...
ConstantBuffer*			g_gameCBO = nullptr;
...
{	
	...
	jobSystemConfig.m_desiredNumOfThreads = std::thread::hardware_concurrency() - 1;
	...
	g_basicSprite = new SpriteSheet(*g_textureLibrary[TEXTURE_BASIC_SPRITE_64_64], IntVec2(64, 64));
	g_theFont = g_theRenderer->CreateOrGetBitmapFont("Data/Fonts/MyFixedFont");

	g_gameCBO = g_theRenderer->CreateConstantBuffer(sizeof(GameConstant));
...
{
	//g_theRenderer->ClearScreen(Rgba8(20, 20, 40, 255));
	g_theGame->Render();
