...
//-----------------------------------------------------------------------------------------------
SoundID AudioSystem::CreateOrGetSound( const std::string& soundFilePath, bool is2D)
{
...
void AudioSystem::UpdateListener(int listenerIndex, const Vec3& listenPosition, const Vec3& listenerForward, const Vec3& listenerUp, const Vec3& listenerVel)
{
	FMOD_VECTOR position;
	position.x = -listenPosition.y;
	position.y = listenPosition.z;
	position.z = -listenPosition.x;
	FMOD_VECTOR velocity;
	velocity.x = -listenerVel.y;
	velocity.y = listenerVel.z;
	velocity.z = -listenerVel.x;
	FMOD_VECTOR forward;
...
SoundPlaybackID AudioSystem::StartSoundAt(SoundID soundID, const Vec3& soundPosition, const Vec3& soundVelocity, bool isLooped, float volume, float balance, float speed, bool isPaused)
{
	size_t numSounds = m_registeredSounds.size();
	if (soundID < 0 || soundID >= numSounds)
		return MISSING_SOUND_ID;
	...
	if (channelAssignedToSound)
	{
		...
		FMOD_VECTOR velocity;
		velocity.x = -soundVelocity.y;
		velocity.y = soundVelocity.z;
		velocity.z = -soundVelocity.x;
...
void AudioSystem::SetSoundPosition(SoundPlaybackID soundPlaybackID, const Vec3& soundPosition, const Vec3& soundVelocity)
{
...
AudioSystemConfig const& AudioSystem::GetConfig() const
{
	return m_config;
}
