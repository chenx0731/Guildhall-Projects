...
objl::Loader*			g_OBJLoader = nullptr;
...
{	
	...
	windowConfig.m_windowTitle = "Doomenstein Platinum";
...
void App::UpdateFromKeyboard()
{
	...
	if (g_theInput->WasKeyJustPressed(KEYCODE_ESC) || g_theInput->GetController(0).WasButtonJustPressed(XBOX_BUTTON_BACK))
	{
		...
		else if (g_theGameState == GameState::MAP_SELECT)
		{
			g_theGames[(int)g_theGameState]->Exit();
			g_theGameState = GameState::ATTRACT;
			g_theGames[(int)g_theGameState]->Enter();
...
void ShowAllKeysOnDevConsole()
{
	...
	g_theDevConsole->AddLine(DevConsole::INFO_MINOR, "Left / Right / Up / Down Arrow 	 - Move Starship");
	g_theDevConsole->AddLine(DevConsole::INFO_MINOR, "Enter 	 - Enter Map");
	g_theDevConsole->AddLine(DevConsole::INFO_MINOR, "Space(in game) 	 - Jump");
	g_theDevConsole->AddLine(DevConsole::INFO_MINOR, "R(when close enough to dog) 	 - Ride");
	g_theDevConsole->AddLine(DevConsole::INFO_MINOR, "S / D	 - Strafe");
	g_theDevConsole->AddLine(DevConsole::INFO_MINOR, "Z / C	 - Elevate");
	g_theDevConsole->AddLine(DevConsole::INFO_MINOR, "E / Q	 - Elevator Up and Down");
