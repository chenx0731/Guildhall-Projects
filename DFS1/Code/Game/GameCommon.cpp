...
void PlayMusicLoop(Music music)
{
	...
	float volume = g_gameConfigBlackboard.GetValue("musicVolume", 0.1f);
	g_theAudio->SetSoundPlaybackVolume(g_lastPlayedMusic[music], volume);
}
...
void PlayMusic(SoundID soundID, Vec3 const& position, Vec3 const& velocity)
{
	SoundPlaybackID playback = g_theAudio->StartSoundAt(soundID, position, velocity);
	float volume = g_gameConfigBlackboard.GetValue("musicVolume", 0.1f);
	g_theAudio->SetSoundPlaybackVolume(playback, volume);
}
