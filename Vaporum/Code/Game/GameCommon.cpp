...
void DrawCircle(Vec2 const& position, float radius, Rgba8 color)
{
	...
	{
		tempVertexes[i * 3].m_position = Vec3(position.x, position.y, 0.02f);
		Vec2 pointOne = Vec2::MakeFromPolarDegrees(angle * i, radius);
		tempVertexes[i * 3 + 1].m_position = Vec3(position.x + pointOne.x, position.y + pointOne.y, 0.02f);
		Vec2 pointTwo = Vec2::MakeFromPolarDegrees(angle * (i + 1), radius);
		tempVertexes[i * 3 + 2].m_position = Vec3(position.x + pointTwo.x, position.y + pointTwo.y, 0.02f);
	}
