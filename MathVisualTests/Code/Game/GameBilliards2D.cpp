...
GameBilliards2D::GameBilliards2D() : Game()
{
...
GameBilliards2D::~GameBilliards2D()
{

}

void GameBilliards2D::Startup()
{
...
void GameBilliards2D::Update(float deltaSeconds)
{
...
void GameBilliards2D::Render() const
{
	g_theRenderer->BeginCamera(m_worldCamera);

	std::vector<Vertex_PCU> fontVerts;
	std::string title = "Mode (F6/F7 for prev/next): Billiards (2D)";
	std::string controls = "F8 to reset & randomize; LMB/RMB set ray start/end; hold T = slow, space = launch billiard, [P]ause, [O]ne step only";
	...
	for (int entityIndex = 0; entityIndex < m_billiards.size(); entityIndex++)
	{
		...
		if (entityIndex != m_billiards.size() - 1)
			m_billiards[entityIndex]->m_rayCastResult.m_didImpact = false;
		if (!m_didImpact)
		{
			m_billiards[entityIndex]->m_rayCastResult.m_didImpact = false;
		}
...
void GameBilliards2D::Shutdown()
{

}

void GameBilliards2D::UpdateEntities(float deltaSeconds)
{
	UNUSED(deltaSeconds);

	for (int i = 0; i < m_billiards.size(); i++) {
		Billiard* tempA = (Billiard*)m_billiards[i];
	...
	for (int i = 0; i < m_billiards.size(); i++) {
		...
		for (int j = 0; j < m_shapes.size(); j++) {
			Billiard* tempB = (Billiard*)m_shapes[j];
	...
	for (int i = 0; i < m_billiards.size(); i++) {
		Billiard* tempA = (Billiard*)m_billiards[i];
		CheckIfOffScreen(tempA->m_center, tempA->m_radius, tempA->m_velocity);
	}
	...
	ShapeEntity2D* secondImpact = nullptr;
	float minSecImpactDis = minImpactDis;
	int impactSecIndex = 0;
	for (int entityIndex = (int)m_billiards.size() - 1; entityIndex >= 0; entityIndex--) {
		ShapeEntity2D*& tempEntity = m_billiards[entityIndex];
		tempEntity->UpdateForRaycast(m_arrowStart, m_arrowEnd);
		RaycastResult2D result = tempEntity->IsImpactedByRaycast();
		if (result.m_didImpact && result.m_impactDis < minSecImpactDis) {
			secondImpact = tempEntity;
			minSecImpactDis = result.m_impactDis;
			impactSecIndex = entityIndex;
			m_impactPos = result.m_impactPos;
			m_impactNormal = result.m_impactNormal;
		}
	}

	if (secondImpact != nullptr) {
		m_didImpact = true;
		m_billiards[impactSecIndex] = m_billiards[m_billiards.size() - 1];
		m_billiards[m_billiards.size() - 1] = secondImpact;
		m_shapes[m_shapes.size() - 1]->m_rayCastResult.m_didImpact = false;
	}

		
	for (int billiardIndex = 0; billiardIndex < m_billiards.size(); billiardIndex++) {
		Billiard* temp = (Billiard*)m_billiards[billiardIndex];
		temp->m_center += deltaSeconds * temp->m_velocity;
	}
	
}

void GameBilliards2D::DeleteGarbageEntities()
{

}

void GameBilliards2D::KillAll()
{

}

void GameBilliards2D::UpdateFromKeyboard(float deltaSeconds)
{
	UNUSED(deltaSeconds);
	...
	if (g_theInput->WasKeyJustPressed(' ') || g_theInput->WasKeyJustPressed('N'))
	{
		...
		billiard->m_radius = 2.f;
		billiard->m_velocity = m_cursor - m_arrowStart;
		billiard->SetPositionAndVelocity(m_arrowStart, m_cursor - m_arrowStart);
...
void GameBilliards2D::UpdateFromController(float deltaSeconds)
{
	UNUSED(deltaSeconds);
...
void GameBilliards2D::RenderAttractMode() const
{
...
void GameBilliards2D::RenderUI() const
{
	g_theRenderer->BeginCamera(m_screenCamera);
...
void GameBilliards2D::RenderEntities() const
{
	g_theRenderer->BeginCamera(m_worldCamera);
...
void GameBilliards2D::RenderPaused() const
{
	g_theRenderer->BeginCamera(m_screenCamera);
...
void GameBilliards2D::DebugRenderEntities() const
{
	g_theRenderer->BeginCamera(m_worldCamera);
	g_theRenderer->EndCamera(m_worldCamera);
}

void GameBilliards2D::EnterGame()
{
	Shutdown();
	Startup();
...
void GameBilliards2D::SetCamera()
{
...
void GameBilliards2D::RandomizeShapes()
{
	m_billiards.clear();
	for (int entityIndex = 0; entityIndex < m_shapes.size(); entityIndex++)
	{
...
bool GameBilliards2D::CheckIfOffScreen(Vec2& position, float radius, Vec2& velocity)
{
	...
	if (position.y + radius > WORLD_SIZE_Y || position.y - radius < 0.f) {
	...
	if ((position.x - radius < 0.f && position.y - radius < 0.f) || (position.y + radius > WORLD_SIZE_Y && position.x + radius > WORLD_SIZE_X)) {
...
void GameBilliards2D::ReturnAttractModeVictory()
{
	ReturnAttractModeNormal();
}

void GameBilliards2D::ReturnAttractModeFailed()
{
	ReturnAttractModeNormal();

}

void GameBilliards2D::ReturnAttractModeNormal()
{
