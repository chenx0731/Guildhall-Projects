...
GameRaycastVsLineSegments::GameRaycastVsLineSegments() : Game()
{
...
void GameRaycastVsLineSegments::Render() const
{
	...
	std::string title = "Mode (F6/F7 for prev/next): Raycast Vs LineSegment (2D)";
...
void GameRaycastVsLineSegments::UpdateEntities(float deltaSeconds)
{
	UNUSED(deltaSeconds);
	m_didImpact = false;
...
void GameRaycastVsLineSegments::DeleteGarbageEntities()
{

}

void GameRaycastVsLineSegments::KillAll()
{
...
void GameRaycastVsLineSegments::RenderAttractMode() const
{
...
void GameRaycastVsLineSegments::RenderUI() const
{
	g_theRenderer->BeginCamera(m_screenCamera);
...
void GameRaycastVsLineSegments::RenderEntities() const
{
	g_theRenderer->BeginCamera(m_worldCamera);
...
void GameRaycastVsLineSegments::RenderPaused() const
{
	g_theRenderer->BeginCamera(m_screenCamera);
...
void GameRaycastVsLineSegments::DebugRenderEntities() const
{
	g_theRenderer->BeginCamera(m_worldCamera);
	g_theRenderer->EndCamera(m_worldCamera);
}

void GameRaycastVsLineSegments::EnterGame()
{
	Shutdown();
	Startup();
...
void GameRaycastVsLineSegments::SetCamera()
{
...
void GameRaycastVsLineSegments::RandomizeShapes()
{
	for (int entityIndex = 0; entityIndex < m_shapes.size(); entityIndex++)
	{
		ShapeEntity2D*& tempEntity = m_shapes[entityIndex];
...
void GameRaycastVsLineSegments::ReturnAttractModeVictory()
{
	ReturnAttractModeNormal();
}

void GameRaycastVsLineSegments::ReturnAttractModeFailed()
{
	ReturnAttractModeNormal();

}

void GameRaycastVsLineSegments::ReturnAttractModeNormal()
{
