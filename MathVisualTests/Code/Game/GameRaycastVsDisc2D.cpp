...
GameRaycastVsDisc2D::GameRaycastVsDisc2D() : Game()
{
...
GameRaycastVsDisc2D::~GameRaycastVsDisc2D()
{

}

void GameRaycastVsDisc2D::Startup()
{
...
void GameRaycastVsDisc2D::Update(float deltaSeconds)
{
...
void GameRaycastVsDisc2D::Render() const
{
	g_theRenderer->BeginCamera(m_worldCamera);

	std::vector<Vertex_PCU> fontVerts;
	std::string title = "Mode (F6/F7 for prev/next): Raycast Vs Disc (2D)";
...
void GameRaycastVsDisc2D::Shutdown()
{

}

void GameRaycastVsDisc2D::UpdateEntities(float deltaSeconds)
{
	UNUSED(deltaSeconds);
	m_didImpact = false;
...
void GameRaycastVsDisc2D::DeleteGarbageEntities()
{

}

void GameRaycastVsDisc2D::KillAll()
{

}

void GameRaycastVsDisc2D::UpdateFromKeyboard(float deltaSeconds)
{
	UNUSED(deltaSeconds);
...
void GameRaycastVsDisc2D::UpdateFromController(float deltaSeconds)
{
	UNUSED(deltaSeconds);
...
void GameRaycastVsDisc2D::RenderAttractMode() const
{
...
void GameRaycastVsDisc2D::RenderUI() const
{
	g_theRenderer->BeginCamera(m_screenCamera);
...
void GameRaycastVsDisc2D::RenderEntities() const
{
	g_theRenderer->BeginCamera(m_worldCamera);
...
void GameRaycastVsDisc2D::RenderPaused() const
{
	g_theRenderer->BeginCamera(m_screenCamera);
...
void GameRaycastVsDisc2D::DebugRenderEntities() const
{
	g_theRenderer->BeginCamera(m_worldCamera);
	g_theRenderer->EndCamera(m_worldCamera);
}

void GameRaycastVsDisc2D::EnterGame()
{
	Shutdown();
	Startup();
...
void GameRaycastVsDisc2D::SetCamera()
{
...
void GameRaycastVsDisc2D::RandomizeShapes()
{
	for (int entityIndex = 0; entityIndex < m_shapes.size(); entityIndex++)
	{
		ShapeEntity2D*& tempEntity = m_shapes[entityIndex];
...
void GameRaycastVsDisc2D::ReturnAttractModeVictory()
{
	ReturnAttractModeNormal();
}

void GameRaycastVsDisc2D::ReturnAttractModeFailed()
{
	ReturnAttractModeNormal();

}

void GameRaycastVsDisc2D::ReturnAttractModeNormal()
{
